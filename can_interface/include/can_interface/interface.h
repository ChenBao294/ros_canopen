...
class SocketCANDriverInterface : public CommInterface, public StateInterface {
public:
    /**
    ...
    virtual ~SocketCANDriverInterface() {}
};


class CanlibDriverInterface : public CommInterface, public StateInterface {
public:
    /**
     * initialize interface
     * 
     * @param[in] hardware_id: Kvaser hardware ID (serial number)
     * @param[in] circuit_id: Kvaser circuit ID (channel number - 1)
     * @param[in] loopback: loop-back own messages
     * @return true if device was initialized succesfully, false otherwise
     */
    virtual bool init(const std::string &hardware_id, const std::string &circuit_id, bool loopback) = 0;
